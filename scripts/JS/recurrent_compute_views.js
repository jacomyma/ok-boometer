...
var t = setInterval(compute_views, 5 * 60 * 1000); // Every 5 minutes
...
function compute_views(){
	...
	function writeWhoTweetedIndex(whoTweetedIndex){
		const dataDirPath = path.join(__dirname, '..','..','app','data')
	...
	function writeUsernameIndex(usernameIndex){
		const dataDirPath = path.join(__dirname, '..','..','app','data')
	...
	function writeTopBoomedTweets(boomings){
		const dataDirPath = path.join(__dirname, '..','..','app','data')
		const now = new Date()
	...
	function writeTopBoomed(boomings){
		const dataDirPath = path.join(__dirname, '..','..','app','data')
		const now = new Date()
		timeRanges.forEach(tr=>{
			let data = d3.nest()
	...
	function writeBoomings(boomings){
		const dataDirPath = path.join(__dirname, '..','..','app','data')
		const now = new Date()
		timeRanges.forEach(tr=>{
